...
The `r Biocpkg("MethylToSNP")` package provides tools for analyzing Illumina's Methylation arrays, specifically the 27k, 450k and EPIC (also known as the 850k) arrays.

In terms of preprocessing and QC assessments the package relies on existing Bioconductor infrastructure provided by `r Biocpkg("minfi")`.  


The input data to this package are

 `RatioSet` - organized by the CpG locus level and mapped to a genome. The data has at least one of two channels: Beta and/or M (logratio of Beta). MethylToSNP needs Beta values and if Beta channel is missing it will be calculated from the M channel.

 Raw IDAT files, representing two different color channels prior to normalization need to be preprocessed by `minfi` using one of the preprocessing methods available, for instance with `minfi::preprocessQuantile()` or `minfi::preprocessRaw()`. 
